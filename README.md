...
当前内置动作包括聊天、连续下蹲示好、移动到坐标/路径点、短时跟随玩家、看向坐标、交互方块、使用背包物品、等待、进食，以及原版协议可识别容器的内容摘要/定量存取和熔炉类方块的检查/加工。`agentFullAutonomy=true` 时 AstrBot 可在类型白名单内自主调用；任务仍受单任务互斥、可取消执行、生命保护、维度/坐标检查、服务端禁区和请求者审计约束。容器操作会返回操作前后物品种类与数量证据，但不把容器内容写入事件历史、任务持久化文件或 RAG。路径点和 `walk`/`rail` 连接保存在当前世界的 `data/mineastr/agent/waypoints.json`，不会进入公共 RAG。
...
0.11.6-dev.2 针对真机中“登录后嵌入室内地板，物理脱困反而把 Bot 推落平台”的情况，遇到服务端权威碰撞时会在第一次卡住就禁止盲目前进/跳跃并请求受限服务端脱嵌。服务端优先把 Bot 垂直抬到同一块地板上方，再按目标方向、原高度和水平距离选择邻近安全位置；纠正坐标后会清除旧路径与移动键，并从新位置重新规划原目标，避免横向逃生点让 Bot 越过室内目标。

0.11.6-dev.3 进一步将服务端提供的 Mod 方块碰撞盒投影到 Mineflayer 物理世界，避免路径规划层知道 BOP 地板可站立、物理层却把它当作空气而下沉。碰撞盒保留半砖等非整块高度，已打开的 Mod 门则会清除本地陈旧碰撞；服务端检测到嵌入时也会立即进入受限脱嵌，不再先等待普通卡死超时。

0.11.6-dev.4 针对真机 BOP 红木门测试中“未开门、触发物理脱困后绕路打破玻璃”的问题，在局部 A* 前主动检测短通道内的服务端描述 Mod 门，开门并确认状态后才重规划。两次无法确认开门时任务会安全失败，不会进入物理脱困。高置信建筑结构由软成本改为绝对禁止破坏，并在 Pathfinder 执行层加入第二道动态拦截，防止陈旧路径绕过规划层保护。

0.11.6-dev.5 根据 dev.4 真机复测继续收紧：门候选不再只扫描 Bot 两格内的本地方块，而是同时使用服务端完整 `nearby_blocks` 列表，支持五格内与目标通道重合的 Mod 门。只要周围十格内存在门/门栅，自动寻路的规划层和实际执行层都会禁止任何挖掘回退，即使玻璃未进入有限的权威方块样本，也不得绕过门破坏建筑。

0.11.6-dev.6 修复了真机专有的 Mod 门识别失败：Mineflayer 的 `blockAt` 要求传入 `Vec3`，此前权威方块列表生成的普通坐标对象会被其拒绝，而测试替身未暴露这一差异。现在所有安全方块读取都会规范化为 `Vec3`，并加入严格契约测试，确保五格内的服务端描述门能够在寻路前被实际读取和打开。

0.11.6-dev.7 新增显式启用的持续陪伴会话、闲时真人化小动作、最近 Agent 事件游标，以及原版兼容容器/熔炉的可验证读写动作。AstrBot 可在每个原子动作后重新观察并做高层规划，主动对话与动作队列独立；未知 Mod GUI 和不可靠的 NeoForge 物品协议会拒绝猜测操作。这仍是开发预览版，不使用 `1.0.0` 编号。
...
0.10.7 修复新登录后长距离 `goto` 可能在约一秒内错误失败的问题。`mineflayer-pathfinder` 在区块尚未加载完成时可能以空路径提前结束 `goto()`；MineAstr 现在会保留当前分段目标，等待后续区块与路径更新或实际位移看门狗超时，不再立即耗尽四次恢复机会。若已经产生朝检查点的可测量进展，仍会立即拼接下一分段。
...
0.11.4 针对“A* 有路径但玩家完全不移动”的执行层卡死加入服务端权威诊断，日志可同时查看姿态、载具、速度、服务端碰撞以及脚下/脚部/头部方块。脱困会在前跳冲刺、后跳、左侧跳和右侧跳间轮换；连续三次真实零位移后才请求服务端受限脱嵌。脱嵌最多水平 2 格、垂直 -1～+2 格，且必须通过已加载区块、世界边界、禁区、支撑面、液体/危险、目标碰撞与分段直线碰撞检查；无安全点时拒绝传送。普通和分层 A* 也获得最多三次全局重规划，RoadWeaver 缺少当前 API 时会熔断至下次服务端重启，不再每分钟重复反射与刷屏。
